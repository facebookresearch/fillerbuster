...
class InputDataset(Dataset):
    ...
    def get_data(self, image_idx: int, image_type: Literal["uint8", "float32"] = "float32") -> Dict:
        ...
        if self.mask_color:
            if image_type == "float32":
                mask_color = torch.tensor(self.mask_color, dtype=torch.float32)
            elif image_type == "uint8":
                mask_color = (torch.tensor(self.mask_color, dtype=torch.float32) * 255.0).to(torch.uint8)
            data["image"] = torch.where(data["mask"] == 1.0, data["image"], torch.ones_like(data["image"]) * mask_color)
