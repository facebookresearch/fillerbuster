        ...
        for i, t in enumerate(scheduler.timesteps):
            ...
            for _ in range(multidiffusion_steps):
                ...
                encoder_hidden_states_ = (
                    encoder_hidden_states.repeat(n // size, 1, 1) if encoder_hidden_states is not None else None
                )
                unconditional_encoder_hidden_states_ = (
                    unconditional_encoder_hidden_states.repeat(n // size, 1, 1)
                    if unconditional_encoder_hidden_states is not None
                    else None
                )
