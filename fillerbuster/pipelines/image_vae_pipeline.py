...
class ImageVAEPipeline(Pipeline):
    ...
    def load_checkpoints(self):
        ...
        if self.config.checkpoint:
            ...
            missing_keys, unexpected_keys = module_wrapper(self.vae).load_state_dict(ckpt["vae_state_dict"], strict=False)
            assert len(missing_keys) == 0, f"Missing keys: {missing_keys}"
            assert len(unexpected_keys) == 0, f"Unexpected keys: {unexpected_keys}"
            self.logger.info("Loading the vae weights done.")
            missing_keys, unexpected_keys = module_wrapper(self.disc).load_state_dict(ckpt["disc_state_dict"], strict=False)
            assert len(missing_keys) == 0, f"Missing keys: {missing_keys}"
            assert len(unexpected_keys) == 0, f"Unexpected keys: {unexpected_keys}"
