...
class TransformerPipeline(Pipeline):
    ...
    def load_checkpoints(self):
        if self.config.image_vae_checkpoint:
            ...
            missing_keys, unexpected_keys = module_wrapper(self.image_vae).load_state_dict(ckpt["vae_state_dict"], strict=False)
        ...
        if self.config.pose_vae_checkpoint:
            ...
            missing_keys, unexpected_keys = module_wrapper(self.pose_vae).load_state_dict(ckpt["vae_state_dict"], strict=False)
            assert len(missing_keys) == 0, f"Missing keys: {missing_keys}"
            assert len(unexpected_keys) == 0, f"Unexpected keys: {unexpected_keys}"
        ...
        if self.config.checkpoint:
            ...
            if "global_seed" in ckpt:
                assert (
                    ckpt["global_seed"] != self.config.global_seed
    ...
    def eval_step(
        ...
        if self.config.ms_dataset_ratio[1] != 0:
            ...
            noise = torch.randn((sview_batch_size, 1, self.image_vae.latent_dim + self.pose_vae.latent_dim, hd, wd), device=image.device)
            self.validation_noise_scheduler.num_inference_steps = self.config.num_test_timesteps
            denoised_input = denoise_sample(
                transformer=module_wrapper(self.transformer),
                scheduler=self.validation_noise_scheduler,
