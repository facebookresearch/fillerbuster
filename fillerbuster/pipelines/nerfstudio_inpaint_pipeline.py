...
class InpaintPipelineConfig(VanillaPipelineConfig):
    ...
    predict_cameras: bool = False
    """Whether to predict cameras."""
    predict_depths: bool = True
    """Whether to predict depths."""
...
class InpaintPipeline(VanillaPipeline):
    ...
    def __init__(
        ...
        self.inpaint_list = []
        self.base_dir = None  # to be populated by the trainer
    ...
    def predict_cameras(self, datas):
        """Predict the cameras poses."""
        image = (
            rearrange(torch.stack([datas[i]["image"] for i in range(len(datas))]), "b h w c -> b c h w").float() / 255.0
        )
        image_mask = rearrange(torch.stack([datas[i]["mask"] for i in range(len(datas))]), "b h w c -> b c h w").float()
        image_pred, origins_pred, directions_pred = self.inpainter.inpaint(
            ...
            multidiffusion_size=8,
            multidiffusion_random=True,
        )
        image_pred = torch.where(image_mask == 1, image, image_pred)
        cameras_pred = fit_cameras(origins_pred, directions_pred, niters=500)

        # reorient the predicted cameras
        poses = torch.cat(
            [cameras_pred.camera_to_worlds, torch.zeros_like(cameras_pred.camera_to_worlds[:, :1])], dim=1
        )
        poses[:, -1, -1] = 1
        poses, _ = camera_utils.auto_orient_and_center_poses(poses.cpu(), method="vertical", center_method="poses")
        scale_factor = 1.0
        scale_factor /= float(torch.max(torch.abs(poses[:, :3, 3])))
        poses[:, :3, 3] *= scale_factor
        cameras_pred.camera_to_worlds = poses[:, :3, :].to(self.device)

        datas_pred = []
        image_pred = (rearrange(image_pred, "b c h w -> b h w c") * 255.0).to(
            self.datamanager.cached_train[0]["image"].dtype
        )
        for i in range(cameras_pred.shape[0]):
            data = {}
            data["image"] = image_pred[i]
            data["mask"] = torch.ones_like(data["image"][..., 0:1])
            data["image_idx"] = i
            datas_pred.append(data)
        return cameras_pred, datas_pred
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if self.start_step is None:
            ...
            if self.config.predict_cameras:
                self.datamanager.train_cameras, self.datamanager.cached_train = self.predict_cameras(
                    self.datamanager.cached_train
                )
                self.camera_handles["predict-cameras"] = draw_cameras(
                    self.viewer_control.viser_server,
                    self.datamanager.train_cameras,
                    self.datamanager.cached_train,
                    prefix="predict-cameras",
