...
class PinholeCameras(nn.Module):
    def __init__(self, n, fxs=None, fys=None, quats=None, ts=None):
        ...
        # camera intrinsics
        if fxs is None:
            fxs = torch.full([n], 1.0)
        if fys is None:
            fys = torch.full([n], 1.0)
        ...
        # camera extrinsics
        if quats is None:
            quats = torch.tensor([[1, 0, 0, 0]], dtype=torch.float32).repeat(self.n, 1)
        if ts is None:
            ts = torch.zeros([self.n, 3], dtype=torch.float32)
    ...
    def backproject(self, us, vs, zs, ndx=None):
        ...
        """
        assert us.shape == vs.shape and vs.shape == zs.shape, (
            f"us ({us.shape}), vs ({vs.shape}), zs ({zs.shape}) must all be same shape"
        )
        if ndx is None:
            points_intrinsics = torch.stack(
                [us / self.fxs[..., None], vs / self.fys[..., None]], -1
            )  # (batch, num_points, 2)
            points_homogenous = nn.functional.pad(
                points_intrinsics, (0, 1, 0, 0), mode="constant", value=1
            )  # (batch, num_points, 3)
        ...
        # points_backprojected[..., 0] = -points_backprojected[..., 0]
        points_backprojected[..., 1] = -points_backprojected[..., 1]
        points_backprojected[..., 2] = -points_backprojected[..., 2]
