...
class BaseRender:
    ...
    """Whether to save the cameras to a file."""
...
class RenderCameraPath(BaseRender):
    ...
    def main(self) -> None:
        ...
        if self.save_cameras:
            camera_path_json = {}
            for i in range(camera_path.shape[0]):
                camera_path_json[i] = camera_path.to_json(i)
            self.output_path.parent.mkdir(parents=True, exist_ok=True)
            with open(str(self.output_path).replace(".mp4", ".json"), "w") as f:
...
@dataclass
class CircleRender(BaseRender):
    """Render a circle trajectory (often not great)."""
    ...
    def main(self) -> None:
        ...
        steps = int(self.frame_rate * self.seconds)
        from fillerbuster.utils.camera_path_utils import random_train_pose, get_focal_len_from_fov

        focal_len = get_focal_len_from_fov(512, 60)
        camera_path = random_train_pose(
            size=(512, 512),
            ...
            center=self.center,
            offset=(0, -1, 0),
            jitter_rotation=False,
            jitter_vertical=False,
            jitter_radius=False,
            jitter_offset=False,
        )
...
Commands = tyro.conf.FlagConversionOff[
    Union[
        ...
        Annotated[CircleRender, tyro.conf.subcommand(name="circle")],
    ]
]
